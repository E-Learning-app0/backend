...
async def read_alternative_exam(module_id: UUID, db: AsyncSession = Depends(get_db)):
    alternative_exam = await get_alternative_exam(db, module_id)
    if alternative_exam:
        return {"content": alternative_exam.content}
